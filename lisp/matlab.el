...
;; I changed this (mine,my) to make pretty colors
(defcustom matlab-keyword-list '("global" "for" "while" "if" "elseif" "else"
				 "endfunction" "return" "break"
				 "switch" "case" "otherwise" "try"
				 "catch" "parfor" "reshape" "sparse" "gmres" "pause" 				 
				 "disp" "sqrt" "linspace" "meshgrid" "zeros" 
				 "real" "imag" "on" "off" "besselj" "besselh" "sum" 
				 "dot" "polyfit" "polyval" "norm" "abs" "size" 
				 "numel" "log" "log10" "min" "max" "transpose"
				 "triu" "trid" "diag" "sin" "cos" "atan" "atan2"
				 "exp" "sinh" "cosh" "asinh" "acosh" "tanh"
				 "atanh" "round")
  "List of keywords for Matlab used in highlighting.
Customizing this variable is only useful if `regexp-opt' is available."
  :group 'matlab
...
(defcustom matlab-handle-graphics-list '("figure" "axes" "axis" "line"
					...
					"setfont" "setcolor" "shading"
					"caxis" "colorbar" "view" "surf" "plot" 
					"pcolor" "legend" "grid" "title"
					"loglog" "hold" "xlabel" "ylabel")
...
;; I changed this (mine,my) -> To add indent
(defconst matlab-block-beg-pre-no-if "for\\|while\\|if\\|switch\\|try\\|parfor"
  "Keywords which mark the beginning of an indented block.
Excludes function.")
		...
		(let ((boc (save-excursion
		  ...
		  (save-excursion
		    ...
		    (let ((parendepth (current-column)))
		      (cond ((and (.= (following-char) ?\( ) ;; I changed this line;; (cond ((and (= (following-char) ?\( )
		    		  (save-excursion
		    		    (matlab-navigation-syntax
		    		      (forward-word -1)
		    		      (looking-at
